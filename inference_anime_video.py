...
#         # Chỉ đồng bộ hóa CUDA nếu đang sử dụng GPU
#         if device == 'cuda':
#             torch.cuda.synchronize(device)
#         else:
#             print("GPU only synchronize")
...
def inference_video(args, video_save_path, device=None, total_workers=1, worker_idx=0):
    ...
    while True:
        ...
        try:
            ...
            # Acquire lock to synchronize CPU operations
            with lock:
                writer.write_frame(output)

        # Optionally, if you're using CUDA, you can still synchronize for GPU operations
        if device == 'cuda':
            torch.cuda.synchronize(device)
...
def main():
    ...
    #parser.add_argument('--output_folder', type=str, default='./output/video/', help='Folder to save the output video/images')
